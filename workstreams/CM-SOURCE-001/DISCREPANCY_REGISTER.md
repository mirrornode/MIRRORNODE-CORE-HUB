...
| D-011 | Canon Gate semantic verification is closed | PR #35 and PR #36 passed configured CI before later Codex review found semantic false-negative paths. Initial PR #37 also passed CI, then Codex found a P1 line-wide exemption bug and a P2 documentary-language false positive. Current PR #37 head `994c806` evaluates prohibited-route occurrences independently, expands bounded documentary wording, and has green Audit + Canon Gate; the earlier PR #37 findings are now outdated, but a new Codex review of the current head is still required for closure | Workflow assurance | ACTIVE CORRECTION | Do not merge #37 until Codex reviews current head `994c806` and no unresolved actionable finding remains; then resolve/supersede outdated review threads with evidence |
| D-012 | `SYSTEM_CONTRACT.md` remains implementation-current | The contract is versioned April 28, 2026, declares LUCIAN port 7700 and `POST /dispatch`, and says it remains operational truth until runtimes change. Current runtime `main` exposes an eight-agent approval-gated plan/approve/execute/trace interface, demonstrating material implementation drift | Canon vs implementation freshness | STALE / RECONCILE | Compare current runtime API/agent registry to the April contract and prepare an explicit successor contract rather than silently reinterpret the old one |
| D-013 | Workstream vocabulary is coherent | Earlier candidate text used `principal`, `authority`, `orchestrator`, `truth`, and `source of truth` with overlapping meanings and mixed external collaborators with MIRRORNODE nodes. Candidate terminology/objective/registry/source-model/handoffs have now been rewritten around explicit classes | Terminology / architecture readability | PARTIALLY RESOLVED | Complete Codex language review of PR #34; apply any factual/terminology findings before promotion work |
...
| D-015 | MOPCON `feat/f04-verified-topology` is compatible with agent-runtime `main` | MOPCON runtime UI calls `/plans/{trace_id}/decision` and expects `pending_approval` + nested planner output. Runtime `main` instead exposes `/plans/{plan_id}/approve` followed by `/plans/{plan_id}/execute`, with `proposed/approved/executed` status. The divergent `build-week/gpt56-operator-runtime` branch matches the MOPCON contract more closely | Cross-repository API contract | VERIFIED FALSE | Choose/reconcile the stable runtime approval contract before MOPCON promotion; do not merge MOPCON as though current runtime `main` were compatible |
| D-016 | Overloaded orchestration/execution vocabulary is confined to one stale README line | Repository search shows `orchestrator` and `execution authority` language distributed across README, agent-stack specifications/review records, protocols, canon checklists/charters/status material, visual direction, Council/MCP material, and the Canon Gate itself | Terminology debt / historical scope | VERIFIED FALSE | Do not mass-rewrite historical or ratified artifacts. Build a scoped successor terminology/role map, mark historical meanings by version/context, and supersede only through the appropriate review/promotion path |
