import sys, os, time
from pathlib import Path
from datetime import datetime, timezone

sys.path.insert(0, str(Path(__file__).parent.parent))
os.environ.setdefault("CANON_ROOT", str(Path(__file__).parent.parent / "canon"))

from fastapi import FastAPI
from canon.contracts.sdk.audit import emit_audit

CHARTER_PATH = Path(os.environ["CANON_ROOT"]) / "charters" / "LUCIAN_PRIME.md"
VERSION = "1.0.0"
BOOT_TIME = datetime.now(timezone.utc).isoformat()

app = FastAPI(title="Lucian", description="Audit Oversight & Lattice Coherence Authority")

@app.get("/health")
def health():
    return {"agent": "lucian", "status": "alive", "boot_time": BOOT_TIME,
            "ts": datetime.now(timezone.utc).isoformat()}

@app.get("/heartbeat")
def heartbeat():
    start = time.time()
    emit_audit(repo="lucian", event_type="agent_invocation",
               actor="system", verdict="SUCCESS",
               evidence={"event": "heartbeat", "error": None})
    latency_ms = round((time.time() - start) * 1000, 2)
    return {
        "engine": "lucian",
        "status": "alive",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "latency_ms": latency_ms,
        "version": VERSION,
        "symbolic_depth": 5,
        "role": "Audit Oversight & Lattice Coherence Authority"
    }

@app.get("/identity")
def identity():
    if not CHARTER_PATH.exists():
        return {"error": "Charter not found"}
    return {"agent": "lucian", "charter": CHARTER_PATH.read_text()}

if __name__ == "__main__":
    import uvicorn
    print("[LUCIAN] Booting — Audit Oversight & Lattice Coherence Authority")
    emit_audit(repo="lucian", event_type="execution", actor="system",
               verdict="SUCCESS", evidence={"event": "boot", "error": None})
    uvicorn.run(app, host="0.0.0.0", port=7700)
