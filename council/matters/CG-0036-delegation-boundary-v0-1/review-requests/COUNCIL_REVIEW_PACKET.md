...
**Normative correction target (immutable):** `59eb560c9233027e245aefb47a258d3ffa4d2e7e`  
**Packet publication note:** this packet is published in the immediately following administrative commit and does not alter the normative target above. Reviewers MUST attest to `59eb560c9233027e245aefb47a258d3ffa4d2e7e`, not to an ancestor and not to the packet-publication commit.  
...
## Dependency-state operator/type closure

The final P2 from review `4999808967` is closed by requiring booleans only with `DEPENDENCY_STATE + UNSEEN` and strings only with `DEPENDENCY_STATE + EQUALS|NOT_EQUALS`; unsupported pairings fail schema validation.
...
- Every position MUST name the immutable normative target `59eb560c9233027e245aefb47a258d3ffa4d2e7e`.
